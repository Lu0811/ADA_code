...
int encontrarVecinoMasCercano(int ciudadActual, vector<bool>& visitado, vector<vector<int>>& distancias) {
...
// Función principal para resolver el TSP utilizando la heurística del vecino más cercano con aristas
vector<int> resolverTSP(vector<vector<int>>& distancias) {
    ...
    while (ruta.size() < numCiudades) {
        int siguienteCiudad = encontrarVecinoMasCercano(ciudadActual, visitado, distancias);
...
int main() {
    ...
    vector<int> rutaOptima = resolverTSP(distancias);
